...
import com.hudomju.swipe.ViewAdapter;

/**
 * A {@link android.view.View.OnTouchListener} that makes the list items in a
 * collection view dismissable. It is given special treatment because by default
 * it handles touches for its list items... i.e. it's in charge of drawing the
 ...
 * {@link SwipeToDismissTouchListener} is paused during list view scrolling.
 * </p>
 * 
 * <p>
 * Example usage:
 ...
 * 		new RecyclerViewAdapter(recyclerView), new SwipeDismissRecyclerViewTouchListener.OnDismissCallback()
 * 		{
...
public class SwipeToDismissTouchListener<SomeCollectionView extends ViewAdapter> implements View.OnTouchListener
{
	...
	private static final String TAG = SwipeToDismissTouchListener.class.getCanonicalName();
	...
	public class RowContainer
	{
		...
		{
			return this.container.getClass().toString();
		}
	}

	/**
	 * The callback interface used by {@link SwipeToDismissTouchListener} to
	 ...
	 * {@link SwipeToDismissTouchListener} is paused during list view
	 * scrolling.</p>
	 * 
	...
	public boolean onTouch(View view, MotionEvent motionEvent)
	{
		...
		switch (motionEvent.getActionMasked())
		{
			...
			case MotionEvent.ACTION_CANCEL:
			{
				...
				{
					// cancel
					mRowContainer.getCurrentSwipingView().animate().translationX(0).alpha(1)
			...
			{
				if (mVelocityTracker == null || mPendingDismiss != null)
				{
					break;
				}
				...
				{
					// dismiss
					final RowContainer downView = mRowContainer; // mDownView
																	// gets
																	// null'd
																	// before
																	// animation
					...
					mRowContainer.getCurrentSwipingView().animate()
							.translationX(dismissRight ? mViewWidth : -mViewWidth).alpha(0).setDuration(mAnimationTime)
							.setListener(new AnimatorListenerAdapter()
							{
								@Override
								public void onAnimationEnd(Animator animation)
								{
									Log.d(TAG, "performDismiss(downView, downPosition)");
			...
			{
				if (mVelocityTracker == null || mPaused || mPendingDismiss != null)
				{
					break;
				}
				...
				if (!mItemSwiped && Math.abs(deltaX) > mSlop && Math.abs(deltaY) < Math.abs(deltaX) / 2)
				{
					mSwiping = true;
					mItemSwiped = true;
				...
				boolean swipingFromLeftToRight = deltaX > 0 ? true : false;

				if (mSwiping)
				{
					Log.d(TAG, "swipe translation changed from " + (swipingFromLeftToRight ? "left" : "right")
							+ " to " + (!swipingFromLeftToRight ? "left" : "right"));
					if (swipingFromLeftToRight)
					{
						((RelativeLayout) mRowContainer.undoContainer).setGravity(Gravity.LEFT);
					}
					else
					{
						((RelativeLayout) mRowContainer.undoContainer).setGravity(Gravity.RIGHT);
					}
					mRowContainer.getCurrentSwipingView().setTranslationX(deltaX - mSwipingSlop);
					// mRowContainer.getCurrentSwipingView().setAlpha(Math.max(0f,
					// Math.min(1f,
	...
	class PendingDismissData implements Comparable<PendingDismissData>
	{
		...
		public String toString()
		{
			return "PendingDismissData(" + position + "," + rowContainer.toString() + ")";
		}
	}
	...
	/**
	 * If a view was dismissed and the undo container is showing it will proceed
	 * with the final dismiss of the item.
	 * 
	...
	/**
	 * If a view was dismissed and the undo container is showing it will undo
	 * and make the data container reappear.
	 * 
	 * @return whether there were any pending rows to be dismissed.
	 */
	public boolean undoPendingDismiss()
	{
		...
		{
			mPendingDismiss.rowContainer.dataContainer.animate().translationX(0).alpha(1).setDuration(mAnimationTime)
					.setListener(null);
	...
	{
		Log.d(TAG, "processPendingDismisses(" + pendingDismissData.toString() + ")");
		mPendingDismiss = null;
		...
		animator.addListener(new AnimatorListenerAdapter()
		{
			...
			public void onAnimationEnd(Animator animation)
			{
				if (mCallbacks.canDismiss(pendingDismissData.position)) mCallbacks.onDismiss(mRecyclerView,
