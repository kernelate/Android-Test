...
public class GlobalBroadcastReceiver extends BroadcastReceiver {
	...
	private static String TAG = GlobalBroadcastReceiver.class.getCanonicalName();
	...
	public void onReceive(Context context, Intent intent) {
		...
		if (Intent.ACTION_BOOT_COMPLETED.equals(action)) {
			...
			if(!Engine.getInstance().isStarted()){
				Log.d(TAG, "Starts the engine from the global broadcast receiver");
			...
			boolean isConnected = activeNetwork != null && activeNetwork.isConnected() && activeNetwork.getState() == NetworkInfo.State.CONNECTED;
			boolean isDisconnected = intent.getBooleanExtra(ConnectivityManager.EXTRA_NO_CONNECTIVITY,Boolean.FALSE);
					
			Log.d(TAG, "isConnected :" + isConnected);
			Log.d(TAG, "isDisconnected :" + isDisconnected);
			
			// Reregister to SIP only if officially connected or disconnected
			if(isConnected || isDisconnected) {
				new RegularSipRegister(context).start();
			}
			
		} else if(NativeService.ACTION_STATE_EVENT.equals(action)){
			boolean isStarted = intent.getBooleanExtra("started", false);
